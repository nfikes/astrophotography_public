...
(function () {
  ...
  // just short of the poles so the up-vector never degenerates.
  const cam = { yaw: 0, pitch: 0, fov: 60 };
  let hovered = -1;
  let hoveredObj = null;
  ...
  function aimAt(dir, clientX, clientY) {
    ...
    let a = ndcX * t * aspect, b = ndcY * t, c = 1;
  ...
  function pixToSky(px, py, w) {
    const fy = w.image_size[1] - py;
    const dx = px - w.crpix[0], dy = fy - w.crpix[1];
  ...
  function skyToPix(ra, dec, w) {
    ...
    return [w.crpix[0] + dx, w.image_size[1] - (w.crpix[1] + dy)];
  }
  ...
  function fsTex(slots) {
    ...
    void main() {
      ...
      v *= edgeFade();
      gl_FragColor = vec4(c * v, v) * (uAccScale * uWeight);
    }`;
  }
  ...
  function projMatrix(w) {
    ...
    const rowA = [], rowB = [];
    for (let i = 0; i < 3; i++) {
      // u = [crpix0 * (p0.d) + (cd11*(eRa.d) - cd01*(eDec.d))/(det*DEG)] / iw
      rowA[i] = (w.crpix[0] * p0[i]
                 + k * (cd[1][1] * eRa[i] - cd[0][1] * eDec[i])) / iw;
      // v flips: py = ih - (crpix1 + dy), so the dy term changes sign.
      rowB[i] = ((ih - w.crpix[1]) * p0[i]
  ...
  const GAIN_MIN = 0.4, GAIN_MAX = 4.0, GAIN_DEFAULT = 1.0;
  ...
  function drawMarkers(VP) {
    ...
    for (const kind of Object.keys(MARKER_KIND)) {
      const list = MARKERS.filter((m) => m.kind === kind);
  ...
  function footprintPass(VP, vis, W, H, feather, outFB, gain) {
    ...
    for (const i of vis) {
      ...
      gl.uniform1f(uWeight, depthWeight(effectiveIntegration(s), accOK && accHalf));
  ...
  function draw() {
    ...
    if (bgVis.length) {
      footprintPass(VP, bgVis, W, H, FEATHER_UV, null, domeGain * BG_GAIN);
    }
    ...
    footprintPass(VP, vis, W, H, FEATHER_UV, null, domeGain);
  ...
  function capLoadDetail(idx, outPxPerDeg) {
    ...
    const haveAll = () => idx.every((i) => {
      const s = shots[i];
      return s.detailLevel >= capTierFor(s, outPxPerDeg) || s.detailLevel >= s.maxLevel;
    });
    return new Promise((resolve) => {
      const tick = () => {
        const late = Date.now() - t0 > CAP_LOAD_TIMEOUT_MS;
        if ((inflight <= 0 && haveAll()) || late) { resolve(!late); return; }
        setTimeout(tick, 120);
      };
      tick();
    });
  ...
  async function capSnap() {
    ...
    try {
      ...
      capStatus(`Gathering ${gathered.length} frame`
                + `${gathered.length === 1 ? "" : "s"}…`);
      const complete = await capLoadDetail(gathered, outPxPerDeg);
      ...
      const rec = {
        ...
        half: accHalf,
        region: reg,
        partial: !complete || bare.length > 0,
        bare: bare.length,
      };
      capKeep(rec);
      ...
      capStatus(`Captured ${crop.w} × ${crop.h} from ${gathered.length} frame`
                + `${gathered.length === 1 ? "" : "s"}`
                + (complete ? "" : " (some detail was still loading)"));
    } catch (e) {
  ...
  function bindInput() {
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      // subtends. The old fixed coefficient over-rotated by about 2.5x,
      // which was liveable at 60 degrees and unusable at half of one, where
      // the whole field is a few hundred pixels of drag wide. Yaw is
      // divided by cos(pitch) because a step along a parallel is that much
      // shorter than the same step on the equator.
      const rect = canvas.getBoundingClientRect();
      const perPx = (cam.fov * DEG) / (rect.height || 1);
      cam.yaw += dx * perPx / Math.max(0.08, Math.cos(cam.pitch));
      cam.pitch = Math.max(-89.5 * DEG,
                           Math.min(89.5 * DEG, cam.pitch + dy * perPx));
      schedule(); updateHud(); refineSoon();
    });
  ...
  window.SkyDome = {
    ...
    lookAt: (raDeg, decDeg, fovDeg) => {
      ...
      return { yaw: cam.yaw, pitch: cam.pitch, fov: cam.fov };
    },
    ...
    // test harness.
    camera: () => ({ yaw: cam.yaw, pitch: cam.pitch, fov: cam.fov,
